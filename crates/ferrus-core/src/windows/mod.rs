//! Windows install tweaks — **the core differentiator of Ferrus** (Phase 4).
//!
...
//! Windows Setup reads it. Two levers only (SPEC-0006): hardware bypass and a
//! local account. Every value is verified against current sources (dated in
//! SPEC-0006), not hardcoded from memory — these bypasses drift per build.
...
impl WindowsTweaks {
    ...
    pub fn any(&self) -> bool {
        self.bypass_hardware || self.local_account.is_some()
    }
}
...
///
/// Deterministic and well-formed. Returns the XML as a string.
///
/// # Errors
///
/// Returns [`Error::InvalidTweaks`] if a local account has an empty name.
pub fn generate_autounattend(tweaks: &WindowsTweaks, profile: &BuildProfile) -> Result<String> {
    ...
    if tweaks.bypass_hardware {
        push_windowspe_bypass(&mut xml, profile);
    }
    if tweaks.local_account.is_some() && profile.bypass_nro_complement {
        push_specialize_bypassnro(&mut xml);
    }
    if let Some(account) = &tweaks.local_account {
        push_oobe_local_account(&mut xml, account);
    }
...
/// Open a `<settings>`/`<component>` block.
fn component_open(xml: &mut String, pass: &str, component: &str) {
...
/// Close a `<component>`/`<settings>` block.
fn component_close(xml: &mut String) {
    xml.push_str("    </component>\n  </settings>\n");
}

/// windowsPE: LabConfig hardware-bypass reg adds (before the compat gate).
fn push_windowspe_bypass(xml: &mut String, profile: &BuildProfile) {
    component_open(xml, "windowsPE", "Microsoft-Windows-Setup");
...
/// specialize: BypassNRO registry complement (offline-account path).
fn push_specialize_bypassnro(xml: &mut String) {
    component_open(xml, "specialize", "Microsoft-Windows-Deployment");
    let command = r#"reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\OOBE" /v BypassNRO /t REG_DWORD /d 1 /f"#;
    xml.push_str("      <RunSynchronous>\n");
    xml.push_str("        <RunSynchronousCommand wcm:action=\"add\">\n");
...
fn push_oobe_local_account(xml: &mut String, account: &LocalAccountSpec) {
    component_open(xml, "oobeSystem", "Microsoft-Windows-Shell-Setup");
    xml.push_str("      <OOBE>\n");
